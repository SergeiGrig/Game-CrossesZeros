...
def show_field(turns) -> None:
    ...
    # FIXME: не работающий код должен быть закомментирован или убран
    for el in turns:
        pass
    # FIXME: в Архитектуре у вас прописаны совсем другие структуры данных для работы с ходами — почему здесь внезапно появляется плоский range? мне казалось, что имеет смысл тестировать ту функцию, которая будет работать с нужной структурой данных
    board = range(1,10)
    print('-------------')
    for i in range(3):
